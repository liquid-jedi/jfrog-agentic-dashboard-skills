...
def respond(decision="allow", reason=""):
    out = {
        "hookSpecificOutput": {
            "hookEventName": event,
...
if re.search(r"\bjf\s+(rt|xr)\s+curl\b", lower):
    if re.search(r"(^|\s)-x\s*(delete|put|patch)\b|(^|\s)-x(delete|put|patch)\b", lower):
        respond("deny", "Blocked destructive JFrog curl method during CISO report workflow.")
    post = re.search(r"(^|\s)-x\s*post\b|(^|\s)-xpost\b", lower)
    if post and "/api/v1/violations" not in lower:
        respond("deny", "Blocked unexpected JFrog POST. Only Xray /api/v1/violations is allowed as a read-style query.")
...
if re.search(r"\bjf\s+rt\s+(del|delete|rm|move|copy|set-props|sp|upload|u)\b", lower):
    if ("upload" in lower or re.search(r"\bjf\s+rt\s+u\b", lower)) and os.environ.get("CISO_ALLOW_REPORT_UPLOAD", "").lower() in {"1", "true", "yes", "on"}:
        respond("allow")
    respond("deny", "Blocked mutating Artifactory command during CISO report workflow. Set CISO_ALLOW_REPORT_UPLOAD=true only for explicit report publication.")
...
if re.search(r"\brm\s+-", lower):
    if not rm_is_scoped(command):
        respond("deny", "Blocked broad file deletion. Cleanup must be scoped to /tmp/ciso-*, explicit report artifacts, or the versioned offline APM zip only.")
...
if re.search(r"\bjf\s+rt\s+(del|delete|rm|move|copy|set-props|sp|upload|u)\b", lower):
    if "upload" in lower or re.search(r"\bjf\s+rt\s+u\b", lower):
        if os.environ.get("CISO_ALLOW_REPORT_UPLOAD", "").lower() in {"1", "true", "yes", "on"}:
            respond("allow")
        respond("ask", "Report publication requires explicit confirmation. Set CISO_ALLOW_REPORT_UPLOAD=true only when requested.")
    respond("deny", "Blocked mutating Artifactory command during CISO report workflow.")

if re.search(r"\brm\s+-", lower):
    allowed_rm = all(fragment in lower for fragment in ("rm", "/tmp/ciso-")) or "executive-report.pdf" in lower or "full-report.pdf" in lower
    if not allowed_rm:
        respond("deny", "Blocked broad file deletion. Cleanup must be scoped to /tmp/ciso-* or explicit report artifacts.")
